  ...
  def spec = suite("Checks")(
    test("transcript/answer checks evaluate in-process") {
      assertTrue(
        ...
        Checks.transcriptCheck(EvalCheck.ToolInputContains("search", "skill://intro"), res).contains(true),
        Checks.transcriptCheck(EvalCheck.ToolInputContains("search", "missing"), res).contains(false),
        res.capturedToolCalls == List(CapturedToolCall("mcp__tb__search", """{"uri":"skill://intro"}""")),
        Checks.transcriptCheck(EvalCheck.AnswerContains("4"), res).contains(true),
        Checks.transcriptCheck(EvalCheck.AnswerContainsIgnoreCase("THE ANSWER"), res).contains(true),
        Checks.transcriptCheck(EvalCheck.AnswerContainsAny(List("missing", "answer")), res).contains(true),
        Checks.transcriptCheck(EvalCheck.AnswerContainsAny(Nil), res).contains(false),
        Checks.transcriptCheck(EvalCheck.AnswerContainsAll(List("answer", "4")), res).contains(true),
        Checks.transcriptCheck(EvalCheck.AnswerContainsAll(List("answer", "missing")), res).contains(false),
        Checks.transcriptCheck(EvalCheck.AnswerContainsAll(Nil), res).contains(true),
